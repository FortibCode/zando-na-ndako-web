...
import type { ApiEnvelope, ZoneLivraison } from "@/lib/types";
...
const INPUT = "w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-sm font-medium focus:border-[#1A2E5A] focus:outline-none";
...
export default function ZonesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await api.get<ApiEnvelope<ZoneLivraison[]>>("/admin/zones");
      setItems(res.data);
    } catch (err) {
  ...
  async function submitCreate() {
    ...
    try {
      ...
      notify("Zone créée."); setCreateOpen(false); load();
  ...
  async function submitEdit() {
    ...
    try {
      ...
      notify("Zone mise à jour."); setEditing(null); load();
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      notify("Zone supprimée."); setDeleteTarget(null); load();
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      {createOpen && (
        ...
        >
          <div className="space-y-4">
            <Field label="Nom de la zone"><input value={createForm.nom_zone} onChange={(e) => setCreateForm((f) => ({ ...f, nom_zone: e.target.value }))} className={INPUT} placeholder="Ex : Bacongo" /></Field>
            <Field label="Ville"><input value={createForm.ville} onChange={(e) => setCreateForm((f) => ({ ...f, ville: e.target.value }))} className={INPUT} /></Field>
            <Field label="Quartiers couverts (séparés par des virgules)"><input value={createForm.quartiers} onChange={(e) => setCreateForm((f) => ({ ...f, quartiers: e.target.value }))} className={INPUT} placeholder="Ex : Moungali, Talangaï" /></Field>
            <Field label="Frais de livraison de base (FCFA)"><input type="number" min={0} value={createForm.frais_livraison_base} onChange={(e) => setCreateForm((f) => ({ ...f, frais_livraison_base: e.target.value }))} className={INPUT} /></Field>
            <div className="grid grid-cols-2 gap-3">
              <Field label="Délai min (min)"><input type="number" min={0} value={createForm.delai_estime_min} onChange={(e) => setCreateForm((f) => ({ ...f, delai_estime_min: e.target.value }))} className={INPUT} /></Field>
              <Field label="Délai max (min)"><input type="number" min={0} value={createForm.delai_estime_max} onChange={(e) => setCreateForm((f) => ({ ...f, delai_estime_max: e.target.value }))} className={INPUT} /></Field>
            </div>
          </div>
        </Modal>
      )}
      ...
      {editing && (
        ...
        >
          <div className="space-y-4">
            <Field label="Nom de la zone"><input value={editForm.nom_zone} onChange={(e) => setEditForm((f) => ({ ...f, nom_zone: e.target.value }))} className={INPUT} /></Field>
            <Field label="Ville"><input value={editForm.ville} onChange={(e) => setEditForm((f) => ({ ...f, ville: e.target.value }))} className={INPUT} /></Field>
            <Field label="Quartiers couverts (séparés par des virgules)"><input value={editForm.quartiers} onChange={(e) => setEditForm((f) => ({ ...f, quartiers: e.target.value }))} className={INPUT} placeholder="Ex : Moungali, Talangaï" /></Field>
            <Field label="Frais de livraison de base (FCFA)"><input type="number" min={0} value={editForm.frais_livraison_base} onChange={(e) => setEditForm((f) => ({ ...f, frais_livraison_base: e.target.value }))} className={INPUT} /></Field>
            <div className="grid grid-cols-2 gap-3">
              <Field label="Délai min (min)"><input type="number" min={0} value={editForm.delai_estime_min} onChange={(e) => setEditForm((f) => ({ ...f, delai_estime_min: e.target.value }))} className={INPUT} /></Field>
              <Field label="Délai max (min)"><input type="number" min={0} value={editForm.delai_estime_max} onChange={(e) => setEditForm((f) => ({ ...f, delai_estime_max: e.target.value }))} className={INPUT} /></Field>
            </div>
            <label className="flex items-center gap-3 rounded-xl border border-slate-200 px-4 py-3 cursor-pointer hover:bg-slate-50 transition-colors">

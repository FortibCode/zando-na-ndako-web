...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (loading) return <LoadingBlock label="Chargement du produit…" />;
  if (!produit) {
    return (
      <div className="text-center py-16">
        <p className="text-sm font-bold text-slate-700">Produit introuvable.</p>
        <Link href="/vendeur/produits" className="inline-flex items-center gap-2 mt-4 text-xs font-extrabold text-[#0B2545] hover:underline">
          <ArrowLeft className="h-4 w-4" /> Retour à mes produits
        </Link>
      </div>
    );
  }
    ...
    <div className="max-w-xl">
      ...
      <div className="flex items-center gap-4 p-4 rounded-2xl border border-slate-200 bg-white mb-6">
        ...
        <div className="text-xs text-slate-500">
          <p className="font-bold text-slate-800">{produit.categorie?.nom_categorie || "Sans catégorie"}</p>
          <p className="flex items-center gap-1.5 mt-1"><Info className="h-3.5 w-3.5" /> Photo et catégorie non modifiables après création.</p>
        </div>
      </div>

      <div className="p-4 rounded-2xl border border-slate-200 bg-white mb-6">
        <p className="text-sm font-black text-slate-900 mb-3">Stock actuel : {produit.quantite_stock}</p>
        <div className="flex gap-2">
          <input type="number" min="1" value={stockDelta} onChange={(e) => setStockDelta(e.target.value)} placeholder="Quantité à ajouter"
          ...
          <Button type="button" variant="secondary" onClick={handleStockAdd} disabled={!stockDelta || stockBusy} className="!py-2">
            {stockBusy ? <Loader2 className="h-4 w-4 animate-spin" /> : <PackagePlus className="h-4 w-4" />} Ajouter
          </Button>
        </div>
      </div>
      ...
      <form onSubmit={handleSave} className="space-y-4">
        ...
        <Button type="submit" variant="primary" disabled={saving} className="w-full !py-3">
          {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : saved ? "Enregistré !" : "Enregistrer les modifications"}
        </Button>
      </form>
    </div>
  );

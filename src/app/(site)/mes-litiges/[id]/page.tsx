  ...
  if (!litige) {
    return (
      <main className="mx-auto w-full max-w-2xl px-4 sm:px-6 lg:px-8 my-16 flex-1 text-center">
        <p className="text-sm font-bold text-slate-700">{t('client.litigeDetail.notFound', 'Litige introuvable.')}</p>
        <Link href="/mes-litiges" className="inline-flex items-center gap-2 mt-4 text-xs font-extrabold text-[#0B2545] hover:underline">
          <ArrowLeft className="h-4 w-4" /> {t('client.litigeDetail.back', 'Mes litiges')}
        </Link>
      </main>
    );
  }
    ...
    <main className="mx-auto w-full max-w-2xl px-4 sm:px-6 lg:px-8 my-8 sm:my-14 flex-1">
      ...
      {(litige.decisions || []).length > 0 && (
        <div className="p-4 rounded-2xl border border-emerald-200 bg-emerald-50/50 mb-4 space-y-2">
          <p className="text-xs font-extrabold text-emerald-700 uppercase tracking-wide">{t('client.litigeDetail.decisionTitle', 'Décision')}</p>
      ...
      <div className="p-4 rounded-2xl border border-slate-200 bg-white">
        <p className="text-sm font-black text-slate-900 mb-3">{t('client.litigeDetail.conversationTitle', 'Conversation')}</p>

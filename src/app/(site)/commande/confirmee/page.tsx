...
function CheckoutConfirmedInner() {
  ...
  return (
    <main className="mx-auto w-full max-w-lg px-4 sm:px-6 lg:px-8 my-14 sm:my-20 flex-1 text-center">
      ...
      <h1 className="text-2xl font-black text-slate-900">{t('client.checkoutConfirmed.title', 'Commande confirmée !')}</h1>
      <p className="text-sm text-slate-500 mt-3">
        {t('client.checkoutConfirmed.messagePrefix', 'Votre commande')} <span className="font-black text-[#0B2545]">{numero}</span> {t('client.checkoutConfirmed.messageMiddle', "d'un montant de")}{" "}
        <span className="font-black text-[#0B2545]">{montant.toLocaleString('fr-FR')} FCFA</span> {t('client.checkoutConfirmed.messageEnd', 'a bien été enregistrée.')}
      </p>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mt-8">
        ...
        >
          <Truck className="h-4 w-4" /> {t('client.checkoutConfirmed.trackOrder', 'Suivre ma commande')}
        </button>
        <Link
          href="/"
        ...
        >
          <Home className="h-4 w-4" /> {t('client.checkoutConfirmed.backHome', "Retour à l'accueil")}
        </Link>
      </div>
    </main>
  );

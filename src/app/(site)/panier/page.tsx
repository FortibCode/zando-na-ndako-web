  ...
  if (items.length === 0) {
    return (
      <main className="mx-auto w-full max-w-2xl px-4 sm:px-6 lg:px-8 my-16 flex-1 text-center">
        ...
        <h1 className="text-xl font-black text-slate-900">Votre panier est vide</h1>
        <p className="text-sm text-slate-500 mt-2">Ajoutez des produits pour commencer.</p>
        <Link href="/produits" className="inline-flex items-center gap-2 mt-6 rounded-full bg-[#0B2545] px-6 py-2.5 text-xs font-extrabold text-white hover:bg-[#061830] transition-colors">
          <ShoppingCart className="h-4 w-4" /> Découvrir nos produits
        </Link>
      </main>
    );
  }
  ...
  return (
    <main className="mx-auto w-full max-w-2xl px-4 sm:px-6 lg:px-8 my-8 sm:my-14 flex-1">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl sm:text-3xl font-black text-slate-900">Mon panier ({count})</h1>
        ...
        {items.map((item) => (
          <div key={item.id} className="flex items-center gap-3 p-3.5 rounded-2xl border border-slate-200 bg-white">
            ...
            <div className="flex items-center gap-2 border border-slate-200 bg-slate-50 rounded-xl px-2 py-1.5 shrink-0">
              <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="p-0.5 text-slate-500 hover:text-slate-900 cursor-pointer" aria-label="Diminuer">
                <Minus className="h-3.5 w-3.5" />
              </button>
              <span className="text-sm font-black text-slate-900 w-5 text-center">{item.quantity}</span>
              <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="p-0.5 text-slate-500 hover:text-slate-900 cursor-pointer" aria-label="Augmenter">
                <Plus className="h-3.5 w-3.5" />
              </button>
            </div>
            <button onClick={() => removeItem(item.id)} className="p-1.5 text-slate-400 hover:text-red-500 transition-colors shrink-0" aria-label="Supprimer">
      ...
      <div className="p-5 rounded-2xl border border-slate-200 bg-white mb-4">
        <p className="text-sm font-black text-slate-900 mb-3">Résumé</p>
      ...
      <div className="p-3.5 rounded-2xl bg-blue-50/60 border border-blue-100 mb-4 text-center">
        <p className="text-xs font-bold text-[#0B2545]">🛵 Livraison en 30–60 min · Zone : Brazzaville uniquement</p>
      </div>
      ...
      >
        ...
        {user?.est_diaspora ? "Commander pour un proche" : "Commander maintenant"}
      </button>
    </main>
  );
}
